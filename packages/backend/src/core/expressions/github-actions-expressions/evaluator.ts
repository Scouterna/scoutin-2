...
import { FilteredArray } from "./filtered_array.ts";
import type { FunctionDefinition } from "./funcs/info.ts";
import { wellKnownFunctions } from "./funcs.ts";
import { idxHelper } from "./idxHelper.ts";
import { TokenType } from "./lexer.ts";
import { equals, falsy, greaterThan, lessThan, truthy } from "./result.ts";

export class Evaluator implements ExprVisitor<data.ExpressionData> {
  private n: Expr;
  private context: data.Dictionary;
  ...
  visitBinary(binary: Binary): data.ExpressionData {
    ...
    switch (binary.operator.type) {
      ...
      case TokenType.LESS_EQUAL:
        return new data.BooleanData(
          equals(left, right) || lessThan(left, right),
        );
    }
  ...
  visitIndexAccess(ia: IndexAccess): data.ExpressionData {
    ...
    if (ia.index instanceof Star) {
      ...
      try {
        ...
        throw new Error(
          `could not evaluate index for index access: ${(e as Error).message}`,
          { cause: e },
        );
      }
  ...
  visitFunctionCall(functionCall: FunctionCall): data.ExpressionData {
    ...
    const f =
      this.functions?.get(functionName) || wellKnownFunctions[functionName];
