...
import { ErrorType, ExpressionError, MAX_PARSER_DEPTH } from "./errors.ts";
import type { FunctionInfo } from "./funcs/info.ts";
import { type ParseContext, validateFunction } from "./funcs.ts";
          ...
          case this.match(TokenType.DOT):
            ...
            if (this.match(TokenType.IDENTIFIER)) {
              ...
              expr = new IndexAccess(
                expr,
                new Literal(new data.StringData(property.lexeme), property),
              );
  ...
  private primary(): Expr {
    switch (true) {
      ...
      case this.match(TokenType.NUMBER):
        return new Literal(
          new data.NumberData(this.previous().value as number),
          this.previous(),
        );

      case this.match(TokenType.STRING):
        return new Literal(
          new data.StringData(this.previous().value as string),
          this.previous(),
        );
